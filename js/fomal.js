...
$.ajax({
  ...
  data: {
    key: 'V22BZ-FL7LX-2N44D-ZRSZ2-7EFRJ-TPBNG',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(116.5898196, 39.786870, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by YYDS Space %c 你正在访问 YYDS Space",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("05/20/2021 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<div style="font-size:13px;font-weight:bold">本站稳定运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<div style="font-size:13px;font-weight:bold">本站稳定运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("font") == undefined) {
  localStorage.setItem("font", "LIKE");
}
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://cdn.niux.plus/LightPicture/2023/03/0f5ce14a2e7924a7.webp);
    --darkmode-bg: url(https://cdn.niux.plus/LightPicture/2023/03/58f50c44dbe18f88.webp);
    --mobileday-bg: url(https://cdn.niux.plus/LightPicture/2023/03/fc62787ae3cd3f3c.webp);
    --mobilenight-bg: url(https://cdn.niux.plus/LightPicture/2023/03/459999ff904a1530.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
